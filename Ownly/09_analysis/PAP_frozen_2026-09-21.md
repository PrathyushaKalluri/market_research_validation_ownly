# Pre-Analysis Plan — FROZEN SNAPSHOT

**Frozen:** 2026-09-21
**Freezes:** `09_analysis/M_pre_analysis_plan.md`
**Study:** Ownly × Gachibowli market-fit validation

---

## Provenance — read this before citing this document as a pre-registration

This snapshot was taken on **2026-09-21**, which is **after** survey data collection and after the
first analysis pass. It is therefore a **retrospective freeze**, and the pre-registration claim it
supports is only as strong as the evidence that each section predates the data. That evidence differs
by section, so it is stated separately rather than claimed uniformly.

### Tier A — cryptographically dated before any data

Sections **§1–§6.6, §7–§10** were committed to git as
`83ff55f — "Add Ownly research project folder"` on **2026-09-14 23:42:19 +0530**.

The survey instrument went live on 2026-09-16 and the first responses arrived after that date. The
git object hash fixes this content to 14 Sep independently of anything asserted in this file, so these
sections are pre-registered in the ordinary sense. Verify with:

```
git show 83ff55f:Ownly/09_analysis/M_pre_analysis_plan.md
```

### Tier B — dated by project log only, not by git

Section **§6.7 (predictions P1–P5)**, the two added cuts in **§7**, the second Holm family in **§8**,
and the **§11** restructure were written on **2026-09-18** and remained uncommitted until this freeze.

`_ops/progress.md` records that they were written pre-data and that the response tracker was
header-only with counts at 0 at the time of writing. **That is a self-attestation, not proof.** The
raw survey export is untracked in git, so no independent timestamp corroborates the ordering, and
2026-09-18 falls inside the fielding window.

**Consequence, and it is deliberate:** P1–P5 are reported as **pre-specified but not independently
verifiable**. They may support a recommendation. They must not carry a confirmatory claim on their
own, and no result in this study rests on P1–P5 alone.

### Tier C — decided after data, recorded as a deviation

The **SRI → drop** decision (`_ops/decisions.md` D17) was taken on 2026-09-21 with data in hand. It is
logged as a **post-data deviation**, not an amendment, and appears in §11 under Deviations. Its
justification is that it selects the option this PAP had already named as the pre-registered default —
i.e. the choice that requires no post-hoc judgement — and not the option that looked best against the
observed data.

### Why publish this rather than a clean freeze date

A freeze dated today with no provenance block would read as a pre-registration and would not be one.
The tiering above is weaker than a true prospective freeze and stronger than an undated plan, which is
an accurate description of this study's actual position. **For any future study: freeze before
fielding, and commit the frozen file.**

---

*Everything below this line is the verbatim content of `M_pre_analysis_plan.md` as of 2026-09-21.*
