...
| # | Task | Depends on | Output | Status |
|---|---|---|---|---|
| 3.1 | **Ownly serviceability check at a Gachibowli address — 10 min, do this first** | — | 4 answers to Claude | ☐ |
| 3.2 | Fill the 4 Group B restaurant names in `restaurant_frame_PREFILLED.csv` | 3.1 | 10 named restaurants | ☐ |
| 3.3 | Import `audit_captures_PREFILLED.csv` into Sheets, share with team | 3.2 | Live audit sheet | ☐ |
| 3.4 | Create the fake-door collector sheet + Apps Script, deploy, paste `/exec` into `CONFIG.ENDPOINT` | — | Collector alive | ☐ |
| 3.5 | Host the page (GitHub Pages or Netlify Drop), **neutral name** | 3.4 | Live URL | ☐ |
| 3.6 | Paste the survey link into `CONFIG.SURVEY_URL` | 3.5, 1.8 | Survey button live | ☐ |
| 3.7 | Run the 10-point QA with `?qa=1` | 3.5 | 10/10 pass | ☐ |
| 3.8 | Post the fake-door link with per-channel UTMs; log each in `experiment_tracker.csv` | 3.7, 2.5 | ≥4 channels posted | ☐ |
| 3.9 | **Audit slot 1 (`wed_dinner`), 19:30–21:30** — 30 captures + screenshots | 3.3 | 30 rows + 30 PNGs | ☐ |
